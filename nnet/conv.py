...
class ConvLayer(object):
    ...
    def initialize(self):
        ...
        b_values=np.zeros((self.filter_shape[0],), dtype='float32')
    ...
    def apply_conv(self, input):
        ...
        #else:
        #    self.conv_output+=self.b.dimshuffle('x', 0, 1, 2)
        
        
        self.conv_output+=self.b.dimshuffle("x", 0, "x", "x")
        if self.pool==True:
            self.pooling=MaxPooling(self.pool_size, self.stride)
            self.conv_output=self.pooling.apply(self.conv_output)
    ...
    def apply_activation(self, pre_activation):
        ...
        @param pre_activation: pre-activation matrix like (W*x + b)
                               the output from apply_conv i.e. self.conv_output
        ---------------------------------------------
