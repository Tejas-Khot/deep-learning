...
class Layer:
        ...
        if self.layer_number == 0:
            ...
            for I in range(0, input_.shape[0], Ratio):
                ...
                for J in range(0, input_.shape[1], Ratio):
                    self.nodes[Nx][Ny].load_input(return_node_input(input_, 
                                                                    [I, J], 
                                                                    Ratio, 
                                                                    self.patch_mode,
                                                                    self.image_type))  
                    # returns inputs to the node located at Position [Nx,Ny]
                    Ny += 1
                Nx += 1
        else:
            Nx = 0  # X coordinate of the current node
                ...
                for J in range(0, len(input_[1]), Ratio):
                    ...
                    for K in range(I, I + Ratio):
                        for L in range(J, J + Ratio):
                            input_temp = np.append(input_temp, 
                                                    np.asarray(input_[K][L].belief))
    ...
    def train_typical_node(self, input_, windowSize, algorithm_choice):
        ...
        if self.layer_number == 0:
            ...
            for I in range(X):
                for J in range(Y):
                    TN.load_input(return_node_input(input_, 
                                                    [I, J], 
                                                    H, 
                                                    self.patch_mode, 
                                                    self.image_type))
